...
void main() {
    ...
    vec3 mField = vec3(0.0);
    vec3 qField = vec3(0.0);
    vec3 nqField = vec3(0.0);
    vec3 cField = vec3(0.0);
    ...
    for (float y = 0.0; y < height; y++) {
        for (float x = 0.0; x < width; x++) {
            ...
            vec3 dPosU = normalize(dPos);
            float q2 = props2.z;

            #if !USE_DISTANCE1
                mField += m2*dPosU/distance2;
                qField += q2*dPosU/distance2;
            #else
                float distance1 = sqrt(distance2);
                mField += m2*dPosU/distance1;
                qField += q2*dPosU/distance1;
            #endif
    ...
    vec3 rForce = vec3(0.0);
    rForce += cField;
    rForce += massConstant * m1 * mField;
    rForce += -chargeConstant * q1 * qField;
    rForce += nuclearChargeConstant * nq1 * nqField;
