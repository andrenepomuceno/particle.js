...
import { scenariosList as defaultScenariosList } from './scenarios.js';
...
export class CoreRuntime {
    constructor({
        graphics,
        scenariosList = defaultScenariosList,
        ...
        this.env = env;
        this.alert = alert;
        this.onSimulationChange = onSimulationChange;

        this.particleSetup = scenariosList[0];
        this.simulationIdx = 0;
        this.simulation = undefined;

        this.setSimulation(new SimulationClass(graphics, new PhysicsClass()), false);
        this.log('simulations loaded: ' + scenariosList.length);
    }

    log(msg) {
        let timestamp = new Date().toISOString();
        console.log(timestamp + ' | ' + this.simulation.cycles + ' | Core: ' + msg);
    }

    setSimulation(nextSimulation, notify = true) {
        nextSimulation.core = this;
        nextSimulation.updatePhysics = (key, value) => this.updatePhysics(key, value);
        this.simulation = nextSimulation;
        if (notify && this.onSimulationChange) {
            this.onSimulationChange(nextSimulation);
        }
    }

    internalSetup(newPhysics) {
        if (newPhysics == undefined) {
            newPhysics = new this.PhysicsClass();
        }

        const nextSimulation = new this.SimulationClass(this.graphics, newPhysics);
        nextSimulation.field = new this.FieldClass(nextSimulation);
        this.setSimulation(nextSimulation);
    }

    setup(idx) {
        this.log('setup idx = ' + idx);

        if (idx != undefined) {
            this.simulationIdx = idx;
            if (idx >= 0 && idx < this.scenariosList.length) {
                this.particleSetup = this.scenariosList[idx];
            } else if (idx == -1) {
                this.particleSetup = this.scenariosList[this.scenariosList.length - 1];
            } else {
                this.log('invalid simulationList index');
                return;
            }
        }

        this.internalSetup();
        this.simulation.setup(this.particleSetup);

        this.log('setup done');
    }

    importParticleListJson(selection, filename, content) {
        this.log('Importing selection ' + filename);
        const physics = this.simulation.physics;

        let imported = this.parseJson(content);
        if (imported == undefined) return;

        if (imported.physics.nuclearForceRange != physics.nuclearForceRange) {
            this.alert('Warning: imported physics constants do not match.');
        }
    ...
    createParticleList(particleList, center = new Vector3(), velocity = new Vector3()) {
        this.log('createParticleList ' + particleList.length + ' ' + center.toArray());

        if (particleList == undefined || particleList.length == 0) return;

        if (particleList.length + this.graphics.particleList.length > this.graphics.maxParticles) {
            this.alert([
        ...
        normalizedList.forEach((p) => {
            ...
            this.graphics.particleList.push(p);
        });
    ...
    updatePhysics(key, value) {
        this.log('updatePhysics key ' + key + ' val ' + value);
        ...
        if (updateShader) {
            this.graphics.drawParticles(simulation.particleList, simulation.physics, true);
        }

        if (fillPhysics) {
            this.graphics.fillPhysicsUniforms();
        }
    }

    findParticle(id) {
        let result = undefined;
        this.graphics.particleList.every((p) => {
            if (p.id == id) {
                result = p;
                return false;
            }
    ...
    updateParticle(particle, key, value) {
        const simulation = this.simulation;
        const physics = simulation.physics;
        this.log('updateParticle key = ' + key + ' val = ' + value + ' particle = ' + particle);
        ...
        switch (key) {
            ...
            case 'colorCharge':
                {
                    const colorCharge = parseColorCharge(value, this.alert);
    ...
    deleteParticleList(list) {
        ...
        this.log('deleteParticleList ' + list.length);
    ...
    particleAutoCleanup(threshold = 4) {
        this.log('particleAutoCleanup threshold = ' + threshold);
    ...
    updateParticleList(parameter, value, list) {
        this.log('updateParticleList param = ' + parameter + ' value = ' + value + ' listLen = ' + (list === undefined ? 'undefined' : list.length));

        const simulation = this.simulation;
        if (list == undefined) {
            list = this.graphics.particleList;
        }
            ...
            case 'center':
                {
                    ...
                    if (center == undefined) {
                        this.alert('Invalid center position.');
                        return;
                    }
                    ...
                    if (physics.enableBoundary && centerVector.length() >= physics.boundaryDistance) {
                        this.alert('Value out of boundaries.');
                        return;
                    }
    ...
    deleteAll() {
        ...
        this.simulation.particleList.splice(0);
        this.simulation.drawParticles();
    }
    ...
    parseJson(content) {
        ...
        if (imported == undefined || imported.physics == undefined || imported.physics.particleList == undefined) {
            this.log('Failed to parse JSON file.');
            return undefined;
        }

        this.log('Loaded particles: ' + imported.physics.particleList.length);

        let newPhysics = new this.PhysicsClass(imported.physics);
    ...
    importJson(filename, content) {
        this.log('importJson ' + filename);

        let graphics = this.simulation.graphics;
